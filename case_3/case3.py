...
for t in np.arange(0, t_max, dt):
    ...
    for i in range(1, N - 1):
        ...
        rho01 = 1 + np.sin(x_shifted1 - np.pi) if np.abs(x_shifted1) < np.pi else 1
        u01 = 0
        rho02 = 1 + np.sin(x_shifted2 - np.pi) if np.abs(x_shifted2) < np.pi else 1
        u02 = 0
